...
void Object::setColor(Json::Value& color){
    // todo try catch
    try{
        if(!color.isMember("ambient"))  throw std::runtime_error("No ambient color provided");
        ...
        if(!color["reflected"].isArray() || color["reflected"].size() != 3) throw std::runtime_error("No correct format given for reflected");
        if(!color["refracted"].isArray() || color["refracted"].size() != 3) throw std::runtime_error("No correct format given for refracted");
        for(int i = 0; i < 3; i++){
